...
UINT32 g_SetSampleNum_BGM_2 = 0;
UINT32 g_SetSampleNum_BGM_3 = 0;
...
void PlaySound(int index, int loopCount)
{
	...
	if (index == Get_BGM_Inf()[BGM_002].fileNum)
	buffer.PlayBegin = g_SetSampleNum_BGM_2;

	if (index == Get_BGM_Inf()[BGM_003].fileNum)
		buffer.PlayBegin = g_SetSampleNum_BGM_3;
...
float GetFrequency(int index)
{
	...
	g_apSourceVoice[index]->GetFrequencyRatio(&frequency_ratio);

	return frequency_ratio;
}

UINT64 GetSourceSample(int index)
{
	XAUDIO2_VOICE_STATE pPPPP;
	g_apSourceVoice[index]->GetState(&pPPPP);

	return pPPPP.SamplesPlayed;
}

void SetFrequency(int index, float ratio)
{
	g_apSourceVoice[index]->SetFrequencyRatio(ratio, 0);
}

void skadmkmfLAS(int index)
{
}//SetEffectParameters
...
void SetSampleNum(SOUND_BGM_NUM bgm_num,UINT32 num)
{
	switch (bgm_num) {
	case BGM_002:
		g_SetSampleNum_BGM_2 = num;
		break;
	case BGM_003:
		g_SetSampleNum_BGM_3 = num;
		break;
	}
}